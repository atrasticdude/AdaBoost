...
#                 left_X,left_y,right_X,right_y = self.split_dataset(X_matrix[samples],y,feature)
...
# Features: X is binary matrix (0/1)
# Labels: y is binary classification (0 or 1)
...
tree = DecisionTree()
root = tree.build_tree(X, y, label=1, depth=2)

# --- Simple print function to see the tree structure ---
def print_tree(node, indent=""):
    if node.value is not None:  # leaf
        print(indent + "Leaf:", node.value)
    else:
        print(indent + f"[Feature {node.feature} = 1]")
        print(indent + "-> Left:")
